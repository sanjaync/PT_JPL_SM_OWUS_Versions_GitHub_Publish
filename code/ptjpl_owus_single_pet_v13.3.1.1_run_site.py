...
"""
ptjpl_owus_single_pet_v13.3.1.1_run_site.py
-------------------------------------------------------------------
Version 13.3.1.1: SM/VOD Grid Search + Analytical OWUS (Single PET)
- Mechanistic SPAC inversion for SM, VOD_C, and VOD_L proxies.
- Removes empirical shape geometries (Linear, Exp, Sig).
- Incorporates VODCA CXKu-Band and L-Band data as moisture proxies.
- Grid search: 72 variants (3 proxies × 1 soil × 2 OWUS × 2 int × 2 dyn × 3 caps).
"""
...
def process_single_nc(site_name, nc_path, df_owus_bf, df_owus_opt, meta_all, df_science_meta, df_vod_c, df_vod_l):
    ...
    out_csv = os.path.join(site_dir, f"{site_suffix}_v13.3.1.1_results.csv")
    metrics_txt = os.path.join(site_dir, f"{site_suffix}_v13.3.1.1_metrics.txt")
    manifest_csv = os.path.join(site_dir, f"{site_suffix}_v13.3.1.1_manifest.csv")

    print(f"\n=== PROCESSING v13.3.1.1: {site_label} ===")
    ...
    # VOD Data Integration (v13)
    if not df_vod_c.empty:
        vc = df_vod_c[df_vod_c['site_match'] == sn.lower()].copy()
    ...
    if not df_vod_l.empty:
        vl = df_vod_l[df_vod_l['site_match'] == sn.lower()].copy()
    ...
    vod_c_norm = np.clip((vod_c_vals - vc_min) / vc_den, 0.0, 1.0) if not np.isnan(vc_min) else np.full(N, np.nan)
    vod_l_norm = np.clip((vod_l_vals - vl_min) / vl_den, 0.0, 1.0) if not np.isnan(vl_min) else np.full(N, np.nan)
    ...
    def calc_le_owus_analytical(proxy_vals, pi_set, soil_comp, int_comp, pet_limit):
        if np.isnan(proxy_vals).all(): return np.full(N, np.nan), np.full(N, np.nan), np.full(N, np.nan)
        beta_full, f_ww = get_beta_vectorized(proxy_vals, pi_set)
    ...
    proxy_opts = {"SM": s_arr, "VODC": vod_c_norm, "VODL": vod_l_norm}
    ...
    for pr_name, pr_v in proxy_opts.items():
            ...
            for o_name, pi_base in owus_opts.items():
                for i_name, i_val in int_opts.items():
                    for d_name, is_dyn in dyn_opts.items():
                        ...
                        bid = f"{pr_name}_{s_name}_{o_name}_PT_{i_name}_Analytical_{d_name}"
                        tc, cc, sc = calc_le_owus_analytical(pr_v, pi_curr, soil_v, act_int, pet_pt)
                        owus_results[f"{bid}_TC"], owus_results[f"{bid}_CC"], owus_results[f"{bid}_SC"] = tc, cc, sc
                        for cap in ["TC", "CC", "SC"]:
                            manifest_records.append({
                                "Model_ID": f"{bid}_{cap}", "Proxy": pr_name, "Soil": s_name, "OWUS": o_name, 
    ...
    # SM regime proxies (from OPT Analytical SM)
...
def process_site(site_name):
    ...
    df_vod_c = pd.read_csv(VOD_CXKU_CSV) if os.path.exists(VOD_CXKU_CSV) else pd.DataFrame()
    if not df_vod_c.empty:
        df_vod_c['date'] = pd.to_datetime(df_vod_c['date'], dayfirst=True, errors='coerce')
        df_vod_c['site_match'] = df_vod_c['site_name'].apply(norm_site_string) if 'site_name' in df_vod_c.columns else df_vod_c['site_id'].apply(norm_site_string)
    df_vod_l = pd.read_csv(VOD_L_CSV) if os.path.exists(VOD_L_CSV) else pd.DataFrame()
    if not df_vod_l.empty:
        df_vod_l['date'] = pd.to_datetime(df_vod_l['date'], dayfirst=True, errors='coerce')
        df_vod_l['site_match'] = df_vod_l['site_name'].apply(norm_site_string) if 'site_name' in df_vod_l.columns else df_vod_l['site_id'].apply(norm_site_string)
