...
"""
ptjpl_owus_single_pet_v12.3.1_run_site.py
-------------------------------------------------------------------
Version 12.3.1: SM-Only Grid Search (Defense-Proofed Dynamic Beta & Shape)
- Removed multiple PET inputs; relies exclusively on base PT-JPL PET.
- Incorporates Exponential and Sigmoid soil water stress functions (Lai et al., 2025).
- Integrates time-adaptive K-Means clustering (30-day rolling window) for Dynamic Beta.
- Includes strict numerical stability floors (Exp denominator, fM division).
- Enforces strict physical threshold hierarchy (s_star > s_wilt).
- CHANGE vs v12.3: "Base" removed from soil_opts grid search. Because the v12.3 OWUS
  architecture converts volumetric water content to normalized PAW (f_REW/s_arr),
  applying dynamic-beta and shape parameters to the implicit PT-JPL soil evaporation
  (which has no explicit soil moisture) is physically inconsistent. "Base" soil output
  is still saved separately as LE_PTJPL_Base. Grid search now has 24 variations (was 48).
"""
...
OUTPUT_DIR = (
    "/home/sanjays/et97_scratch2/oldscratch/Ozflux_data_full/ptjpl_ptjplsm/"
    "output_L6_OWUS_SinglePET_v12.3.1_DynamicBeta"
)
...
def simple_kmeans(X, n_clusters=2, max_iter=100):
    """Native K-Means implementation based on Euclidean distance for Dynamic Beta clustering."""
    np.random.seed(42)
...
def process_single_nc(site_name, nc_path, df_owus_bf, df_owus_opt, meta_all, df_science_meta):
    ...
    out_csv = os.path.join(site_dir, f"{site_suffix}_v12.3.1_results.csv")
    metrics_txt = os.path.join(site_dir, f"{site_suffix}_v12.3.1_metrics.txt")
    manifest_csv = os.path.join(site_dir, f"{site_suffix}_v12.3.1_manifest.csv")

    if os.path.exists(out_csv):
        print(f"\n=== SKIP: {site_label} (exists) ===")
        return

    print(f"\n=== PROCESSING v12.3.1: {site_label} ===")
    ...
    if not ms.empty:
        ma = (ms.set_index(pd.to_datetime(ms["date"])).sort_index().reindex(df_d.index).interpolate(limit_direction="both"))
    ...
    # Ensure all required columns exist to avoid KeyError when filtering
    for r in req:
        if r not in inp.columns:
            inp[r] = np.nan
    ...
    # -----------------------------------------------------------------
    # FIX: TIME-ADAPTIVE DYNAMIC REGIME CLUSTERING (30-day Rolling)
    # -----------------------------------------------------------------
    ...
    # Heuristic dynamic parameter shifts based on regimes 
    ...
    owus_results = {}; manifest_records = []; constraint_diag = []

    def calc_le_owus(s_star_arr, s_wilt_arr, beta_ww_arr, soil_comp, int_comp, pet_limit, bid_base, shape_t='lin', shape_p=0.0):
        # Apply Shape Parameters to Beta
        s_norm = np.clip((s_arr - s_wilt_arr) / np.maximum(s_star_arr - s_wilt_arr, 1e-6), 0.0, 1.0)
        
        if shape_t == 'lin':
            f_OWUS_val = s_norm
        elif shape_t == 'exp':
            # FIX: Stabilized denominator to prevent exponential blowup
        ...
        f_OWUS = np.where(s_arr > s_star_arr, beta_ww_arr, beta_ww_arr * f_OWUS_val)
        f_OWUS = np.where(s_arr <= s_wilt_arr, 0.0, f_OWUS)
        
        f_TRM = (1.0 - weight) * fM + weight * f_OWUS
        
        # FIX: Regularize fM to prevent division by near-zero at low LAI
    ...
    # =================================================================
    # REDUCED GRID SEARCH: 24 Variations (SM-only; Base removed)
    # -----------------------------------------------------------------
    # Rationale: The original PT-JPL base soil evaporation uses an
    # *implicit* soil moisture scalar (RH^VPD). The Purdy et al. PTJPLSM
    # update replaces this with explicit volumetric water content converted
    # to normalised PAW (s_arr / f_REW). Applying the dynamic-beta and
    # shape-parameter machinery to the implicit "Base" soil term breaks
    # the physical logic — s_arr thresholds (s_wilt, s_star) and beta_ww
    # are calibrated for explicit SM, not for RH^VPD.
            ...
            for i_name, i_val in int_opts.items():
                for sh_name, (sh_t, sh_p) in shape_opts.items():
                    for d_name, is_dyn in dyn_opts.items():
                        ...
                        plim = pet_pt
                        p_name = "PT" # Maintained for downstream column compatibility
                        
                        if is_dyn:
                            ss_arr = ss * dyn_mult_ss
                            sw_arr = sw * dyn_mult_sw
                            # FIX: Strict floor to prevent broken normalization hierarchy
                        ...
                        bid = f"LE_{s_name}_{o_name}_{p_name}_{i_name}_{sh_name}_{d_name}"
                        tc, cc, sc = calc_le_owus(ss_arr, sw_arr, bw_arr, soil_v, act_int, plim, bid, sh_t, sh_p)
    ...
    output["Precip"] = prec
    # Precip-based day type flags
    output["dry_day"] = dry_mask.astype(int)
    output["wet_day"] = wet_mask.astype(int)

    # Soil-moisture regime flags (aligned with PAW normalization)
    output["SM_PAW"] = s_arr
    output["SM_regime_threshold_set"] = "OPT" # hardcoding for consistency in v12.3.1 script given the logic above
    ...
    output["Regime_Code"] = regimes # Note: In v12.3.1, this is the K-means regime (0 or 1), NOT the classical SM regime code (0,1,2). We keep it for tracking the dynamic regimes.

    # Rebuilding classical SM regime string and code for comparison
    ...
    # SM regime masks (numpy arrays aligned to df_run / output)
    ...
    with open(metrics_txt, "w") as f:
        ...
        f.write(f"SM regime threshold set: OPT | "
                f"s_wilt={opt_sw:.3f} | s_star={opt_ss:.3f}\n")
        ...
        # NEW: Soil-moisture regime blocks
...
def process_site(site_name):
    print(f"=== BATCH PROCESSING: {site_name} (v12.3.1) ===")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python ptjpl_owus_single_pet_v12.3.1_run_site.py <site_name>")
        sys.exit(1)
